...
function sendEmailInBackground(name, cafe, type, message) {
    ...
    html: `
      <div style="font-family:sans-serif;max-width:500px;margin:0 auto;border:1px solid #E8D0A8;border-radius:12px;overflow:hidden">
        ...
        <div style="padding:24px;background:#FEF9F4">
          <p style="margin:0 0 12px"><b>From:</b> ${name}</p>
          <p style="margin:0 0 12px"><b>Café:</b> ${cafe}</p>
          <p style="margin:0 0 12px"><b>Type:</b> ${type}</p>
...
router.post('/', feedbackLimiter, async (req, res) => {
  try {
    ...
    const safeName    = (name    || '').trim().slice(0, 60)  || 'Anonymous VITian';
    const safeCafe    = (cafe    || '').trim().slice(0, 60)  || 'General';
    const safeType    = (type    || '').trim().slice(0, 80)  || 'General Feedback';
    ...
    sendEmailInBackground(safeName, safeCafe, safeType, safeMessage);
